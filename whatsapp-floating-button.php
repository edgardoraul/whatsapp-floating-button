...
function wfb_settings_page() { ?>
    <div class="wrap">
        <h1>Configuración del Botón Flotante de WhatsApp</h1>
        <form method="post" action="options.php">
            ...
            <table class="form-table">
                <tr valign="top">
                    <th scope="row">Número de WhatsApp (con código de país)</th>
                    <td><input type="text" name="wfb_phone" value="<?php echo esc_attr(get_option('wfb_phone')); ?>" placeholder="5491123456789" class="regular-text" /></td>
                </tr>
                <tr valign="top">
                    <th scope="row">Posición Vertical</th>
                    <td>
                        <select name="wfb_position_vertical">
                            <option value="bottom" <?php selected(get_option('wfb_position_vertical'), 'bottom'); ?>>Abajo</option>
                            <option value="top" <?php selected(get_option('wfb_position_vertical'), 'top'); ?>>Arriba</option>
                        </select>
                    </td>
                </tr>
                <tr valign="top">
                    <th scope="row">Posición Horizontal</th>
                    <td>
                        <select name="wfb_position_horizontal">
                            <option value="right" <?php selected(get_option('wfb_position_horizontal'), 'right'); ?>>Derecha</option>
                            <option value="left" <?php selected(get_option('wfb_position_horizontal'), 'left'); ?>>Izquierda</option>
                        </select>
                    </td>
                </tr>
                <tr valign="top">
                    <th scope="row">Distancia Vertical (px)</th>
                    <td><input type="number" name="wfb_offset_vertical" value="<?php echo esc_attr(get_option('wfb_offset_vertical', 20)); ?>" /></td>
                </tr>
                <tr valign="top">
                    <th scope="row">Distancia Horizontal (px)</th>
...
add_action('wp_footer', function() {
    ...
    ?>
    <a href="<?php echo esc_url($whatsapp_url); ?>" target="_blank" rel="noopener" class="wfb-button" style="<?php echo esc_attr($style); ?>">
